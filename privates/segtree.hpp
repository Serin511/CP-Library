#ifndef PRIVATES_SEGTREE
#define PRIVATES_SEGTREE 1
#include <bits/stdc++.h>
using namespace std;
template <class T, class L, auto op, auto mapping, auto push, T tid, L lid>
struct segtree {
    explicit segtree(int n=0) : segtree(vector<T>(n, tid)) {}
    explicit segtree(const vector<T> &v): n(v.size()) {
...
private:
    ...
    void apply(int i, L l) {
        mapping(l, st[i]);
        if (i<b) lz[i]=push(lz[i], l);
    }
    void down(int i) {
        apply(2*i, lz[i]);
        apply(2*i+1, lz[i]);
