...
class Expander:
    atcoder_include = re.compile(
        r'#include\s*["<]((atcoder|privates)/[a-z_]*(|.hpp))[">]\s*')

    include_guard = re.compile(r'#.*(ATCODER_[A-Z_]*_HPP|PRIVATES_[A-Z_]*)')

    std_include = re.compile(r'#include\s*[<"][^"]*[">]\s*')
    ...
    def is_ignored_line(self, line) -> bool:
        ...
        if line.strip().startswith('//'):
            return True
        if self.std_include.match(line) and not self.atcoder_include.match(line) and 'stdc++' in line:
            return True
        if line.strip().startswith('using namespace') and 'std' in line:
            return True
        return False
